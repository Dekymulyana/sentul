...
class Penjualan extends MY_Controller
{
	...
	public function transaksi()
	{
		...
		if($level == 'admin' OR $level == 'kasir')
		{
			...
			else
			{
				...
				$dt['perusahaan']= $this->m_perusahaan->get_all();
				$dt['nama_resto_toko']= $this->m_merk_barang->get_all();
	...
	public function transaksi_cetak()
	{
		...
		if( ! empty($id_pelanggan))
		{
			...
			$alamat = $this->m_perusahaan->get_baris($id_perusahaan)->row()->alamat;
				...
				$pdf->Ln();
				$pdf->Cell(120, 5, $nama_perusahaan, 0, 0, 'L');
				$pdf->Cell(50, 5, 'Penjualan Tunai', 0, 0, 'L');
				$pdf->Ln();
				$pdf->Cell(120, 5, $alamat, 0, 0, 'L');
				$pdf->Cell(50, 5, 'Costumer : ', 0, 0, 'L');
				$pdf->Ln();
				$pdf->Cell(15, 5, 'NPWP :', 0, 0, 'L');
				$pdf->Cell(105, 5, $npwp, 0, 0, 'L');
				$pdf->Cell(50, 5, $pelanggan, 0, 0, 'L');
				$pdf->Ln();
				$pdf->Cell(120, 5, '', 0, 0, 'L');
				$pdf->Cell(85, 5, $alamat_pelanggan, 0, 0, 'L');
				$pdf->Ln();

				$pdf->Cell(15, 5, 'Phone :', 0, 0, 'L');
				$pdf->Cell(85, 5, $no_telphone, 0, 0, 'L');
				$pdf->Ln();
				$pdf->Cell(25, 5, 'No Invoice : ', 0, 0, 'L');
				$pdf->Cell(85, 5, $no_invoice, 0, 0, 'L');
				$pdf->Ln();
				$pdf->Cell(25, 5, 'Tgl Invoice : ', 0, 0, 'L');
				$pdf->Cell(85, 5, date('d-M-Y', strtotime($tanggal)), 0, 0, 'L');
				$pdf->Ln();
				$pdf->Ln();
				...
				$pdf->Cell(40, 5, 'Harga', 0, 0, 'L');
				$pdf->Cell(45, 5, 'Qty', 0, 0, 'L');
				$pdf->Cell(30, 5, 'Jumlah', 0, 0, 'R');
				$pdf->Ln();
		...
		foreach($_GET['kode_barang'] as $kd)
		{
			if( ! empty($kd))
			{
				...
				$pdf->Cell(60, 5, $nama_barang, 0, 0, 'L');
				$pdf->Cell(40, 5, str_replace(',', '.', number_format($_GET['harga_satuan'][$no])), 0, 0, 'L');
				$pdf->Cell(40, 5, $_GET['jumlah_beli'][$no], 0, 0, 'L');
				$pdf->Cell(25, 5, str_replace(',', '.', number_format($_GET['sub_total'][$no])), 0, 0, 'R');
				$pdf->Ln();
		...
		$pdf->Cell(130, 5, '-----------------------------------------------------------------------------------------------------------------------------------------------------------------', 0, 0, 'L');
		$pdf->Ln();
		$pdf->Cell(25, 4, date('d-M-Y', strtotime($tanggal)).'/KASIR', 0, 0, 'L');
		$pdf->Cell(75, 5, 'Qty', 0, 0, 'R');
		$pdf->Cell(30, 5, str_replace(',', '.', number_format($qty)), 0, 0, 'R');
		$pdf->Cell(35, 5, 'Sub Total', 0, 0, 'R');
		$pdf->Cell(25, 5, str_replace(',', '.', number_format($grand_total)), 0, 0, 'R');
		$pdf->Ln();
		$ppn = $grand_total/10;
		$pdf->Cell(165, 5, 'PPN 10%', 0, 0, 'R');
		$pdf->Cell(25, 5, str_replace(',', '.', number_format($ppn)), 0, 0, 'R');
		$pdf->Ln();

		$pdf->Cell(165, 5, 'Grand Total', 0, 0, 'R');
		$pdf->Cell(25, 5, str_replace(',', '.', number_format(($ppn + $grand_total))), 0, 0, 'R');
		$pdf->Ln();

		$pdf->Cell(130, 5, '-----------------------------------------------------------------------------------------------------------------------------------------------------------------', 0, 0, 'L');
		$pdf->Ln();

		$pdf->Cell(50, 5, 'Catatan : ', 0, 0, 'L');
	...
	public function ajax_pelanggan()
	{
		if($this->input->is_ajax_request())
		{
			...
			$json['nama']			= ( ! empty($data->nama)) ? $data->nama : "<small><i>Tidak ada</i></small>";
