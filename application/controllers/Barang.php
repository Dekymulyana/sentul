...
class Barang extends MY_Controller
{
	...
	public function ajax_toko()
	{
		if($this->input->is_ajax_request())
		{
			$id_resto_toko = $this->input->post('id_toko_resto');
			$this->load->model('m_merk_barang');

			$data = $this->m_merk_barang->get_baris($id_resto_toko)->row();
			$json['alamat']			= ( ! empty($data->alamat)) ? $data->alamat : "<small><i>Tidak ada</i></small>";
			$json['keterangan']			= ( ! empty($data->keterangan)) ? $data->keterangan : "<small><i>Tidak ada</i></small>";
		...
		if($level == 'admin' OR $level == 'inventory')
		{
			if($_POST)
			{
				...
				foreach($_POST['kode'] as $kode)
				{
					...
					$this->form_validation->set_rules('nama['.$no.']','Nama Barang #'.($no + 1),'trim|required|max_length[60]');
	...
	public function list_merek_json()
	{
		...
		foreach($query->result_array() as $row)
		{
			...
			$nestedData[]	= $row['nomor'];
			$nestedData[]	= $row['id_merk_barang'];
			$nestedData[]	= $row['merk'];
			$nestedData[]	= $row['alamat'];
			$nestedData[]	= $row['alamat_2'];
			$nestedData[]	= $row['alamat_3'];
			$nestedData[]	= $row['fax'];
			$nestedData[]	= $row['no_telphone'];
		...
		if($level == 'admin' OR $level == 'inventory')
		{
			if($_POST)
			{
				...
				if($this->form_validation->run() == TRUE)
				{
					...
					$insert = $this->m_merk_barang->tambah_merek($data);
					if($insert)
					{
						echo json_encode(array(
							'status' => 1,
							'pesan' => "<div class='alert alert-success'><i class='fa fa-check'></i> <b>".$this->input->post('merek')."</b> berhasil ditambahkan.</div>"
						));
					}
					else
					{
	...
	public function edit_merek($id_merk_barang = NULL)
	{
		if( ! empty($id_merk_barang))
		{
			...
			if($level == 'admin' OR $level == 'inventory')
			{
				...
				'provinsi' =>  $this->input->post('provinsi'),
			'no_telphone'=>$this->input->post('no_telphone'));
							$insert = $this->m_merk_barang->update_merek($id_merk_barang, $data);
							if($insert)
							{
								echo json_encode(array(
									'status' => 1,
					...
					{
						$data = array(
							'merek' => $this->m_merk_barang->get_baris($id_merk_barang)->row() ,
						'alamat'=> $this->m_merk_barang->get_baris($id_merk_barang)->row(),
					'keterangan'=> $this->m_merk_barang->get_baris($id_merk_barang)->row());
						//$dt['merek'] = $this->m_merk_barang->get_baris($id_merk_barang)->row();
						//$dt['alamat'] = $this->m_merk_barang->get_baris($id_merk_barang)->row();
						$this->load->view('barang/merek/merek_edit', $data);
					}
				}
			}
		}
