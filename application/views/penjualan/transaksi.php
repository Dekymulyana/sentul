...
<div class="container-fluid">
	<div class="panel panel-default">
		<div class="panel-body">
			...
			<div class='row'>
				<div class='col-sm-3'>
					<div class="panel panel-primary" id='PerusahaanArea'>
						<div class="panel-heading"><i class='fa fa-building'></i> Template Resto/Toko</div>
						<div class="panel-body">
							<div class="form-group">
								<label>Nama Resto/Toko</label>

								<select name='id_toko_resto' id='id_toko_resto' class='form-control input-sm' style='cursor: pointer;'>
									<option value=''>-- Pilih Resto/Toko --</option>
									<?php
									if($nama_resto_toko->num_rows() > 0)
									{
										foreach($nama_resto_toko->result() as $p)
										{
											echo "<option value='".$p->id_merk_barang."'>".$p->merk." (".$p->provinsi.") </option>";
										}
									}
									?>
								</select>
							...
							<div class="form-horizontal">
								<div class="form-group">
									...
									<div class="col-sm-8">
										<div style="margin-top: 7px;" id='keterangan_resto_toko'><small><i>Tidak ada</i></small></div>
					...
					<div class="panel panel-primary" id='PerusahaanArea'>
						<div class="panel-heading"><i class='fa fa-building'></i> Informasi Perusahaan</div>
						<div class="panel-body">
							<div class="form-group">
								<label>Nama PT</label>

								<select name='id_perusahaan' id='id_perusahaan' class='form-control input-sm' style='cursor: pointer;'>
									...
									if($perusahaan->num_rows() > 0)
									{
										...
										{
											echo "<option value='".$p->id_perusahaan."'>".$p->nama_perusahaan."</option>";
										}
									}
									?>
								</select>
							...
							<div class="form-horizontal">
								<div class="form-group">
									...
									<div class="col-sm-8">
										<div id='npwp'><small><i>Tidak ada</i></small></div>
					...
					<div class="panel panel-primary">
						<div class="panel-heading"><i class='fa fa-file-text-o fa-fw'></i> Informasi Struk</div>
						<div class="panel-body">

							<div class="form-horizontal">
								<!--<div class="form-group">
									<label class="col-sm-4 control-label">Telphone</label>
									<div class="col-sm-8">
										<input type='text' name='no_telphone' class='form-control input-sm' id='no_telphone' value="" <?php echo $readonly; ?> required>
									</div>
								</div>
								<div class="form-group">
									<label class="col-sm-4 control-label">Fax</label>
									<div class="col-sm-8">
										<input type='text' name='fax' class='form-control input-sm' id='fax' value="" <?php echo $readonly; ?> required>
									</div>
								</div>-->
								<div class="form-group">
									<label class="col-sm-4 control-label">Table</label>
									<div class="col-sm-8">
										<input type='text' name='table' class='form-control input-sm' id='table' value="" <?php echo $readonly; ?> required>
									</div>
								</div>
								<div class="form-group">
									<label class="col-sm-4 control-label">Invoice/Queue</label>
									<div class="col-sm-8">
										<input type='text' name='nomor_nota' class='form-control input-sm' id='nomor_nota' value="" <?php echo $readonly; ?> >
									</div>
								</div>
								<div class="form-group">
									<label class="col-sm-4 control-label">Cashier/OP</label>
									<div class="col-sm-8">
										<input type='text' name='kasir_op' class='form-control input-sm' id='kasir_op' value="" <?php echo $readonly; ?> required>
									</div>
								</div>
								<div class="form-group">
									<label class="col-sm-4 control-label">Waiter</label>
									<div class="col-sm-8">
										<input type='text' name='waiter' class='form-control input-sm' id='waiter' value="" <?php echo $readonly; ?> required>
									</div>
								</div>
								<div class="form-group">
						...
						<div class="panel-body">
							...
							<div class="form-horizontal">
								...
								<div class="form-group">
									...
									<div id='nama_pelanggan' style="color:#fff;"><small><i>Tidak ada</i></small></div>
								</div>
							</div>
				...
				<div class='col-sm-9'>
					...
					<div class='alert alert-info TotalBayar'>
						...
						<h5>Sub Total : <span id='TotalBayar'>Rp. 0</span></h5> <br>
						<h5>Pajak : <span id='Pajak'>Rp. 0</span></h5> <br>
						<h2>Total : <span id='TotalBayarSeluruh'>Rp. 0</span></h2>
...
$('#id_toko_resto').change(function(){
		if($(this).val() !== '')
		{
			$.ajax({
				url: "<?php echo site_url('barang/ajax-toko'); ?>",
				type: "POST",
				cache: false,
				data: "id_toko_resto="+$(this).val(),
				dataType:'json',
				success: function(json){
					$('#alamat_resto_toko').html(json.alamat);
					$('#keterangan_resto_toko').html(json.keterangan);
		...
		{
			$('#alamat_resto_toko').html('<small><i>Tidak ada</i></small>');
			$('#keterangan_resto_toko').html('<small><i>Tidak ada</i></small>');
...
$('#id_perusahaan').change(function(){
		...
		{
			$.ajax({
				url: "<?php echo site_url('perusahaan/ajax-perusahaan'); ?>",
				type: "POST",
				cache: false,
				data: "id_perusahaan="+$(this).val(),
				dataType:'json',
				success: function(json){
					$('#npwp').html(json.npwp);
		...
		{
			$('#npwp').html('<small><i>Tidak ada</i></small>');
			$('#alamat').html('<small><i>Tidak ada</i></small>');
			$('#provinsi').html('<small><i>Tidak ada</i></small>');
...
function BarisBaru()
{
	...
	var Baris = "<tr>";
		...
		Baris += "<td><input type='text' class='form-control' id='jumlah_beli' name='jumlah_beli[]'  disabled></td>";
		Baris += "<td>";
...
$(document).on('keyup', '#jumlah_beli', function(){
	...
	$.ajax({
		...
		success: function(data){
			if(data.status == 1)
			{
				var SubTotal = parseInt(Harga) * parseFloat(JumlahBeli);
...
function HitungTotalBayar()
{
	...
	$('#TabelTransaksi tbody tr').each(function(){
		if($(this).find('td:nth-child(6) input').val() > 0)
		{
			...
			Pajak = parseInt(Total)/10;
			TotalSeluruh = parseInt(Total) + parseInt(Pajak);
		}
	});

	$('#TotalBayar').html(to_rupiah(Total));
	$('#Pajak').html(to_rupiah(Pajak));
	$('#TotalBayarSeluruh').html(to_rupiah(TotalSeluruh));
	$('#TotalBayarHidden').val(TotalSeluruh);
	$('#SubTotalHidden').val(Total);
...
function CetakStruk()
{
	if($('#TotalBayarHidden').val() > 0)
	{
		if($('#UangCash').val() !== '')
		{
			...
			var FormData = "id_perusahaan="+encodeURI($('#id_perusahaan').val());
			FormData += "&pencarian_kode="+encodeURI($('#pencarian_kode').val());
			FormData += "&id_kasir="+encodeURI($('#id_kasir').val());
			FormData += "&table="+encodeURI($('#table').val());
			FormData += "&id_toko_resto="+encodeURI($('#id_toko_resto').val());
			FormData += "&tanggal="+encodeURI($('#tanggal').val());
			FormData += "&nomor_nota="+encodeURI($('#nomor_nota').val());
			FormData += "&waiter="+encodeURI($('#waiter').val());
			FormData += "&no_telphone="+$('#no_telphone').val();
			FormData += "&id_pelanggan="+$('#id_pelanggan').val();
			FormData += "&alamat="+$('#alamat').val();
			FormData += "&npwp="+$('#npwp').val();
			FormData += "&" + $('#TabelTransaksi tbody input').serialize();
			FormData += "&cash="+$('#UangCash').val();
			FormData += "&catatan="+encodeURI($('#catatan').val());
			FormData += "&grand_total="+$('#SubTotalHidden').val();
			FormData += "&id_pelanggan="+$('#id_pelanggan').val();
			FormData += "&kasir_op="+$('#kasir_op').val();
			FormData += "&fax="+$('#fax').val();


			window.open("<?php echo site_url('Cetak/?'); ?>" + FormData,'_blank');
